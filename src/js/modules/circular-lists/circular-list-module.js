...
export const circularListModule = {
  id: 'circular-lists',
  ...
  renderEditor({ state, icons, escapeText }) {
    ...
    return `
      ...
      <label class="field-label" for="node-input">Elementos (do início em diante)</label>
      ...
      <p id="node-help" class="field-help">Informe os valores do início em diante, separados por vírgula, espaço ou ponto e vírgula. O último valor aponta de volta para o início. Deixe em branco para uma lista vazia. Máximo: ${MAX_NODES} nós.</p>
  ...
  renderConfig({ state }) {
    ...
    if (algorithmId === 'deletarNo') {
      return `
        <div class="panel-heading compact-heading"><div><span class="eyebrow">Parâmetros</span><h2 id="config-title">Escolha o valor a excluir</h2></div></div>
        <label class="config-field"><span>Valor (chave)</span><input id="item-input" type="number" value="${config.item}" data-module-input="item" /><small>A busca começa em início e percorre o ciclo.</small></label>`;
    }
    return '';
  },
