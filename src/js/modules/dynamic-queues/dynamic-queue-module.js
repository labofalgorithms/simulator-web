...
const DEFAULT_DATA = { values: [9, 42, 51] };
const MAX_NODES = 8;
...
export const dynamicQueueModule = {
  id: 'dynamic-queues',
  name: 'Filas Dinâmicas',
  version: '1.0',
  ...
  createInitialConfig(data) {
    return { enqueueValue: data.values.length ? data.values[data.values.length - 1] + 1 : 15 };
  },
  ...
  renderEditor({ state, icons, escapeText }) {
    ...
    return `
      ...
      <div class="input-action-row">
        <input id="node-input" class="text-input ${state.inputError ? 'invalid' : ''}" value="${escapeText(state.draft)}" placeholder="9, 42, 51" aria-describedby="node-help node-error" data-module-input="draft" data-module-enter="apply" />
        <button class="button primary" type="button" data-module-action="apply-data">${icons.check}<span>Aplicar</span></button>
      </div>
      <p id="node-help" class="field-help">Informe os valores do início para o fim, separados por vírgula, espaço ou ponto e vírgula. Deixe em branco para uma fila vazia. Máximo: ${MAX_NODES} nós.</p>
  ...
  renderConfig({ state }) {
    ...
    return `
      ...
      <label class="config-field"><span>Valor</span><input id="enqueue-value-input" type="number" value="${state.config.enqueueValue}" data-module-input="enqueue-value" /><small>Um novo nó será ligado ao final. Não há verificação de fila cheia.</small></label>`;
  },
